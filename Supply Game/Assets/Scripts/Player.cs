...
//	public EdgeCollider2D hookEdgeCollider; // I'm essentially using a tractor beam here, so no colliders

	private CircleCollider2D playerArea;
//	private Vector2[] edgePoints = new Vector2[2]; // Deprecated, why would I anyway want a collider in a tractor beam?
...
//		Debug.Log (hookEdgeCollider.points.GetValue(1));
	}
	...
	void Update () 
	{
		...
		if (lineRenderer.enabled) 
		{
			...
			// Updating the collider (I may use a Trigger instead)
//			edgePoints [0] = Vector2.zero; // Done in the inspector, never changes in localPosition
//			edgePoints [1] = cargoScript.hookColliderPoint.position - hookPosition.transform.position;
//			Debug.Log ("newEdgePoints: " + edgePoints);

//			hookEdgeCollider.points = edgePoints;
		}
	}
...
//			hookEdgeCollider.enabled = true;
	...
	void GenerateRope()
	{
		...
		if (hookTargetCollider != null) 
		{
			...
			// TODO: I have to learn how to use a layerMask to avoid these workarounds
			cargoScript = hookTargetCollider.gameObject.GetComponent<Cargo> ();
		...
		for (int i = 0; i < links; i++) 
		{
			...
			// Both work fine, I don't know wich one is more resource efficient
//			ropeLinks [i] = link;
...
//		hookEdgeCollider.enabled = false;
